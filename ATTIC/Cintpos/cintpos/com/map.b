...
$(  LET blocklist 	= rootnode ! rtn_blklist
    ...
    $( writes("Map of free and allocated store*N*N")
       ...
       $(   LET s = blocks!i
            ...
            writef("%i8 words", s)
            UNLESS free DO writef(" %tF", a+s-5)

            FOR id = 1 TO tasktab!0 DO
            { LET t = tasktab!id
              LET base = a+1
              UNLESS t LOOP
              IF base = t             DO writef(" Task %n's TCB", id)
              IF base = t!tcb_seglist DO writef(" Task %n's segment list", id)
              IF base = t!tcb_sbase   DO writef(" Task %n's stack", id)
              IF base = t!tcb_gbase   DO writef(" Task %n's globals", id)
            }
            IF a = mapstack     DO writef(" %tF MAP's stack", a+s-5)
            IF a = mapcode      DO writef(" %tF MAP's code", a+s-5)
            IF a = (blocks-1)   DO writef(" %tF MAP's workspace", a+s-5)
            IF a = (rootnode-2) DO writes(" Rootnode")
