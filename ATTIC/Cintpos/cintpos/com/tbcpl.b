...
18/2/04
Now first using current directory then directories in POSROOT
in GET directives.
...
s_slct=26; s_of=27                   // Inserted 11/7/01
...
s_valdef=141; s_vecdef=142; s_constdef=143; s_const=144
s_fndef=145; s_rtdef=146
...
sourcenamev:227; sourcefileno:228; sourcefileupb:229
...
{ LET treesize = 0
   AND argv = VEC 50
   AND argform = "FROM/A,TO/K,VER/K,SIZE/K,TREE/S,NONAMES/S,*
                 *D1/S,D2/S,OENDER/S,EQCASES/S,BIN/S,XREF/S,DEFS/S"
   LET stdout = output()
   ...
   xrefing := argv!11                     // XREF
   defsing := argv!12                     // DEFS
   ...
   { LET b = VEC 64/bytesperword
      ...
      // Sourcefile 0 is always the FROM argument filename
      // others are GET files
      sourcenamev!0 := argv!0
      sourcefileno := 0
      FOR i = 1 TO sourcefileupb DO sourcenamev!i := 0 // Done for safety
      chcount, lineno := 0, (sourcefileno<<24) + 1
      rch()
...
fin:
   ...
   IF ocodeout      DO { selectoutput(ocodeout)
                          UNLESS ocodeout=stdout DO endwrite()
                       }
   IF gostream      DO { selectoutput(gostream)
...
{ nlpending := FALSE
   ...
   { SWITCHON ch INTO
      ...
      { CASE '*n':
         ...
         CASE '[': symb := s_sbra;      BREAK
         CASE ']': symb := s_sket;      BREAK
         ...
         CASE ':': rch()
              ...
              IF ch=':' DO { symb := s_of;  BREAK  }  // Inserted 11/7/01
              symb := s_colon
              RETURN
...
//writef("trying: GET *"%s*" in POSHDRS*n", charv)
   // First look in the current directory
   stream := pathfindinput(charv, "POSROOT")

   UNLESS stream DO
   { LET dirname = "g/"
     LET ext     = ".h"
     LET n = 0                  // Form:  g/<name>.h
     ...
     filename%0 := n
//sawritef("trying: GET *"%s*" in POSROOT*n", filename)
     stream := pathfindinput(filename, "POSROOT")  // MR 18/2/04
   }
   ...
   { LET len = charv%0
     ...
     IF str FOR i = 0 TO len DO str%i := charv%i
     sourcefileno := sourcefileno+1
     sourcenamev!sourcefileno := str
   }
   getstreams := list4(getstreams, sourcestream, lineno, ch)
   sourcestream := stream
   selectinput(sourcestream)
   lineno := (sourcefileno<<24) + 1
   rch()
}
...
{ LET d = value(ch)
   decval := ch='_' -> 0, d
   IF decval>=radix DO synerr("Bad number")
...
{ LET res, id = 0, 0
   ...
   { LET ln = lineno
      ...
      !ptr := list5(s_constdef, 0, id, kexp, ln)
      ptr := @h2!(!ptr)
...
{ LET a, op = 0, symb
   ...
   { DEFAULT: synerr("Error in expression")
      ...
      CASE s_slct: { LET len, sh, offset = 0, 0, 0  // Inserted 11/7/01
                     ...
                     RESULTIS list4(s_slct, len, sh, offset)
                   }
   ...
   { LET op = symb
      ...
      { DEFAULT:       RESULTIS a
         ...
         CASE s_sbra:   b := rnexp(0)   // Inserted 11/6/02
                        UNLESS symb=s_sket DO synerr("']' missing")
                        lex()
                        a := list3(s_vecap, a, b)
                        LOOP
...
{ LET n = rnamelist()
   ...
   { CASE s_lparen:
        { LET a = 0
           ...
           IF symb=s_be DO
           { lex()
              RESULTIS list6(s_rtdef, n, a, rcom(), 0, ln)
           }
 
           IF symb=s_eq RESULTIS list6(s_fndef, n, a, rnexp(0), 0, ln)
      ...
      CASE s_eq:
           ...
           { UNLESS h1!n=s_name DO synerr("Name required before = VEC")
              RESULTIS list4(s_vecdef, n, rnexp(0), ln)
           }
           RESULTIS list4(s_valdef, n, rexplist(), ln)
   }
}
...
GLOBAL  {
wrn:212  
sourcenamev:227  // Fileno to string mapping
...
isconst:335; evalconst:336; transname:337; xref:338
nextlab:343; labnumber:344; translate:345; newblk:346
dvec:360; dvece:361; dvecp:362; dvect:363
caselist:365; casecount:366
context:369; comline:370; procname:371
...
{ LET c = cellwithname(x)
   ...
   TEST h2!c=s_global THEN { LET gn = h3!c
                              ...
                              IF defsing DO writef("G%i3 = %s*n", gn, @h3!x)
                           }
...
{ LET a, b = 0, 0
   ...
   { CASE s_name:
        { LET c = cellwithname(x)
           IF h2!c=s_manifest DO
           { LET k = h3!c
             IF xrefing DO xref(x, "M:", k, s_const)
             RESULTIS k
           }
           trnerr("%s must be a manifest constant", @h3!x)
           RESULTIS 0
        }
...
{ LET op = h1!x
  ...
  { DEFAULT: ENDCASE
    ...
    CASE s_number: writef("%n", h2!x);         RETURN 
...
// Copyright  M.Richards  25 Sept 2003.
