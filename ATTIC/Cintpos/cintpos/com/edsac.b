...
// Modified to run under Cintpos so that recordings can be made.
...
GLOBAL {
 ...
 cohandid   // The task number of the console handler
}
...
{ LET argv = VEC 30
  ...
  writef("*nEDSAC Simulator -- Type ? for help*n*n")
//abort(1234)
  // Use exclusive input mode while in the edsac shell
...
AND rch() = VALOF
{ LET ch = sendpkt(notinuse, cohandid, Action_exclusiverdch, 0, 0)
  wrch(ch)
  deplete(cos)
  RESULTIS ch
}
...
rec:
  ...
  { // Main debug loop
    ...
    IF ch='*n' DO { writef("# "); deplete(cos) }  // The prompt
...
sw: op := capitalch(ch)
    ...
    { DEFAULT:    error("Unknown command")
      ...
      CASE '?':
        writef("*n*n")
        writef("?             Print list of debug commands*n")

        writef("123  .125  #1011  #.1011  'c  17-bit constants*n")
        writef("123S .125S #1011S #.1011S     17-bit constants*n")
        writef("123L .125L #1011L #.1011L     35-bit constants*n")
        writef("123F .125F #1011F #.1011F     71-bit constants*n")

        writef("**k +k -k      Multipy/Add/Subtract constant k*n")
        writef("/ ^           Divide/multiply the current value by 10*n")
        writef("~             Negate the current value*n")
        writef("< >           Shift the current value left/right one place*n")

        writef("$<s>          Set the default printing style to <s>*n")
        writef("                <s> = b   binary integer*n")
        writef("                <s> = d   decimal integer*n")
        writef("                <s> = f   decimal fraction*n")
        writef("                <s> = i   instruction*n")
        writef("=             Print the current value in current style*n")

        writef("LS LL LF      Change the length of the current value*n")

        writef("A H P         Get value from Acc, H or SCR*n")
        writef("Ma MSa MLa    Get a 17 or 35 bit value from memory*n")
        writef("SA SH         Store the current value in Acc or H*n")
        writef("Ja            Jump to a, ie set SCR to a*n")
        writef("Sa            Store the current value in memory address a*n")
        writef("Ia            Assemble instructions in location a, a+1,..*n")

        writef("Tn<s> TSn<s>  Print n consecutive 17-bit locations, *
               *style <s>*n")
        writef("TLn<s>        Print n consecutive 35-bit words, style <s>*n")

        writef("F<name>       Set the paper tape filename*n")
        writef("Q             Quit*n")
        writef("R             Load initial orders *
               *and clear registers SCR, H and Acc*n")
        writef("Z             Set all 1024 memory locations to zero*n")

        writef("DP            Toggle dump of SCR and the next order*n")
        writef("DR<s>         Toggle dump of the operand, style <s>*n")
        writef("DSH<s> DLH<s> Toggle dump of H, style <s>*n")
        writef("DSA<s> DLA<s> DFA<s> Toggle dump of Acc, style <s>*n")
        writef("DSa<s>        Toggle dump of 17-bit memory location, *
               *style <s>*n")
        writef("DLa<s>        Toggle dump of 35-bit memory word, style <s>*n")
        writef(";             Print requested values*n")

        writef("B  Ba  Ua     List, set or unset breakpoints*n")
        writef("C             Continue normal execution*n")
        writef("\             Execute one instruction*n")
        ch := '*n'
        LOOP
      ...
      CASE 'I': // Assemble instructions
        ...
        { LET instr, addr, len = 0, 0, 0
          ...
          ch := capitalch(rch()) REPEATWHILE ch=' '
      ...
      { LET size, st = 'S', style
        ...
        ELSE { FOR i = 0 TO n-1 DO
               { writef("%i4: ", addr+i)
                 ldws(addr+i)
//writef("*nw0=%bF w1=%bF*n", w0, w1); abort(1000)
                 printw(st)
                 writef("*n")
               }
             }
      ...
      { LET size = ch
        ...
        { DEFAULT:  ldvs(getint() & 1023); GOTO sw
          CASE 'S': ch := capitalch(rch())
                    ldvs(getint() & 1023); GOTO sw
          CASE 'L': ch := capitalch(rch())
      ...
      CASE '=':
        //writef("*n%bF %bF %bF %bF %bB %c*n", v0, v1, v2, v3, v4>>4, vsize)
        writef("*n")
        v2w()
        printw(style)
        writef("*n")
      ...
      CASE 'R':
        reset()
        writef("*nInitial orders loaded and registers cleared*n")
        ch := '*n'
        LOOP

      CASE 'Z':
        FOR i = 0 TO 1023 DO mem!i := 0
        writef("*nMemory cleared*n")
        ch := '*n'
        LOOP
      ...
      { LET addr = ?
        ...
        { // List the breakpoints
          UNLESS ch='*n' DO wrch('*n')

          writef("Breakpoints:*n")
      ...
      CASE 'F':
      { LET len = 0
        writef("*nFile name for Edsac paper tape reader: ")
...
lookup:
      ...
      CASE 'C':
        writef("*n")
        code := interpret()
        //writef("*ncode = %n*n", code)
        SWITCHON code INTO
        { DEFAULT:

          CASE 0:
            writef("*nSuccessful termination of the program*n")
            ch := '*n'
            LOOP
          ...
          CASE 2: // Breakpoint return
            writef("*nBreakpoint at %n*n", scr)
            ENDCASE

          CASE 3: // Unknown instruction
            writef("*nUnknown instruction:*n")
            printinstr(mem!scr)
            ENDCASE
...
{ LET instr = mem!scr
  ...
  { LET flags = mflags!addr
    IF (flags & -2) = 0 LOOP
    writef("%i4: ", addr)
    IF (flags & mf_sb) ~= 0 DO { writef(" "); ldws(addr); printw('B') } // SaB
    IF (flags & mf_sd) ~= 0 DO { writef(" "); ldws(addr); printw('D') } // SaD
    IF (flags & mf_sf) ~= 0 DO { writef(" "); ldws(addr); printw('F') } // SaF
    IF (flags & mf_si) ~= 0 DO { writef(" "); ldws(addr); printw('I') } // SaI

    IF (flags & mf_lb) ~= 0 DO { writef(" "); ldwl(addr); printw('B') } // LaB
    IF (flags & mf_ld) ~= 0 DO { writef(" "); ldwl(addr); printw('D') } // LaD
    IF (flags & mf_lf) ~= 0 DO { writef(" "); ldwl(addr); printw('F') } // LaF
    wrch('*n')
  } 
}
...
{ LET frac = FALSE
  ...
  { DEFAULT:  error("Bad number")
    ...
    CASE '5': CASE '6': CASE '7': CASE '8': CASE '9': 
      ...
      { mulwby10()
        adddig(ch-'0')
        //writef("*n%bF %bF %bF %bF %bB*n", w0, w1, w2, w3, w4>>4)
        ch := capitalch(rch())
      }
      ENDCASE
    ...
    { x0, x1, x2, x3, x4 :=      0, 0, 0, 0, 0
      ...
      {
//wrch('*n')
  //writef("x: %bF %bF %bF %bF %bF*n", x0, x1, x2, x3, x4)
  //writef("w: %bF %bF %bF %bF %bF*n", w0, w1, w2, w3, w4)
  ...
  //writef("w: %bF %bF %bF %bF %bF*n", w0, w1, w2, w3, w4)
      ...
      x2w()
      roundw()
  //writef("w: %bF %bF %bF %bF %bF result rounded*n", w0, w1, w2, w3, w4)
      frac := TRUE
      ENDCASE
    }
...
//writef("w: %bF %bF %bF %bF %bF rounded*n", w0, w1, w2, w3, w4)
  w2x()

  // Calculate the remainder
  mulwby10()
//writef("s: %bF %bF %bF %bF %bF*n", s0, s1, s2, s3, s4)
//writef("-: %bF %bF %bF %bF %bF*n", w0, w1, w2, w3, w4)
  negw()
...
{ LET digv = VEC 30
//writef("printw: wsize=%c style=%c*n", wsize, style)
//writef("%bF %bF %bF %bF %bB %c*n", w0, w1, w2, w3, w4>>4, wsize)
//abort(1000)
  ...
  { DEFAULT: error("Bad size/style combination: %c/%c", wsize, style)
    ...
    CASE 'S'<<8 | 'B':
      writef("%bF%b2",w0, w1>>13)
      RETURN   

    CASE 'L'<<8 | 'B': 
      writef("%bF%b2 %b1 %bC%b5", w0, w1>>13, w1>>12 & 1, w1&#xFFF, w2>>10)
      RETURN   

    CASE 'F'<<8 | 'B':  
      writef("%bF%b2 %b1 %bC%b5", w0, w1>>13, w1>>12 & 1, w1&#xFFF, w2>>10)
      writef(" %bA%bF%bB*n", w2&#x3FF, w3, w4>>4)
      RETURN   
...
//writef("w: %bF %bF %bF %bF %bF*n", w0, w1, w2, w3, w4)
  ...
  FOR i = 26-n TO 25 DO wrch(str%i)
}
...
{ // Write the decimal fraction
  ...
  //writef("w: %bF %bF %bF %bF %bF*n", w0, w1, w2, w3, w4)
  //writef("+: %bF %bF %bF %bF %bF*n", t0, t1, t2, t3, t4)
...
{ LET addr = instr>>1 & 2047 // Include the unused bit
  LET op = instr>>12 & 31
  writef("%b5_%b1_%bA_%b1   ",
     op, instr>>11 & 1, instr>>1 & 1023, instr&1)
  writef("%c%i4%c  // ", prf2asc(op), addr, (instr&1)=0 -> 'S', 'L')
  SWITCHON op INTO
  { DEFAULT: RETURN

    CASE  3: // E
      writef("if acc>=0 goto %n", addr); RETURN
    CASE  4: // R
      writef("acc >>= %n", lbpos(instr)); RETURN
    CASE  5: // T
      writef("mem[%n] = acc; acc = 0", addr); ENDCASE
    CASE  6: // Y
      writef("acc += 2****-36"); RETURN
    CASE  7: // U
      writef("mem[%n] = acc", addr); ENDCASE
    CASE  8: // I
      writef("mem[%n] = rd()", addr); RETURN
    CASE  9: // O
      writef("wr(mem[%n])", addr); RETURN
    CASE 12: // S
      writef("acc -= mem[%n]", addr); ENDCASE
    CASE 13: // Z
      writef("stop"); RETURN
    CASE 17: // F
      writef("verify last output"); RETURN
    CASE 21: // H
      writef("H = mem[%n]", addr); ENDCASE
    CASE 22: // N
      writef("acc -= H ** mem[%n]", addr); ENDCASE
    CASE 25: // L
      writef("acc <<= %n", lbpos(instr)); RETURN
    CASE 26: // X
      writef("no operation", addr); RETURN
    CASE 27: // G
      writef("if acc<0 goto %n", addr); RETURN
    CASE 28: // A
      writef("acc += mem[%n]", addr); ENDCASE
...
{ IF ch=11 DO { figshift := TRUE;  RETURN }
  ...
  TEST figshift THEN wrch(fig2asc(ch))
                ELSE wrch(let2asc(ch))
}
