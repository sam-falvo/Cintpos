...
//writef("trying: GET *"%s*" in POSHDRS*n", charv)
   stream := pathfindinput(charv, "POSHDRS")
   ...
   { LET dirname = "g/"
     ...
     filename%0 := n
//writef("trying: GET *"%s*" in POSROOT*n", filename)
     stream := pathfindinput(filename, "POSROOT")  // MR 10/9/03
   }
