...
//sawritef("DEBUG: entered  pkt=%n*n", pkt)
  ...
  set.process.name("Debug Task") // MR 3/2/03
  ...
  bpt_addr     := rootnode!rtn.bptaddr
  bpt_instr    := rootnode!rtn.bptinstr
...
sw:
  ...
  { DEFAULT: error()
    ...
    CASE '?':
       ...
       writes("Z  (P1B9)  Set breakpoint 9 at return of current fn*n")
       writes("C          Continue normal execution -- disabled*n")
       writes("\          Execute one instruction   -- disabled*n")
    ...
    CASE 'H': // Hold/Release currently selected task
              ...
              { LET state = tcb.state!ctcb
                LET held = state & #b0010
                tcb.state!ctcb := state NEQV #b0010
                writef("*nTask %n %s*n",
                        tcb.taskid!ctcb, held -> "released", "held")
                GOTO nxt
              }
...
{ LET id = -1
  LET st = regs!r_st
  LET letter = '?'
  IF ctcb DO id, letter := ctcb!tcb.taskid, ctcb!tcb.active -> 'a', 'd'
  IF st=1 DO letter := 'k'
  IF st=2 DO letter := 'b'
  IF st=3 DO letter := 'i'
...
{ LET base, lim, n = ?, ?, ?
   ...
   { DEFAULT:   error()
      ...
      CASE 'W': base, lim := ctcb, tcb.upb;         ENDCASE
...
{ LET type, base = 'A', 0
   ...
   IF ctcb <= a <= ctcb+tcb.upb         DO type, base := 'W', ctcb
...
{ // IF id<0 select the current tcb and the trap registers
  ...
  regs := @tcb.regs!t       // regs belonging to non current task
