...
msecs) to dlib.  The clock device is now sent packets by
...
{ // Read and return a floating point number from the
  ...
  // If successful, the result is the 32-bit representation
  // of the floating point number, and result2 is zero.
  // On failure the result is zero and result2 to {\tt-1}.
  ...
  // necessary
  LET ch = rdch()
  LET dcount = 0    // Number of digits
  LET ecount = 0    // Number of digits after the 9 significand digits
  LET dpos = -1     // Position of the decimal point if any
...
//sawritef("readflt: dealing with significand*n"); checkpos(ch)
    ...
    { DEFAULT: BREAK
      ...
      CASE '5': CASE '6': CASE '7': CASE '8': CASE '9':
        dcount := dcount+1
        // Do not change the significand if 9 digits have
        // been processed following the leading zeroes.
        TEST val < 100_000_000
        THEN val := 10*val + ch - '0'
        ELSE ecount := ecount+1  // A digit after 9 significand
                                 // digits.
//sawritef("readflt: was a digit*n")
        ENDCASE
...
//sawritef("readflt: Exited from significand*n"); checkpos(ch)
  ...
  // eg 00123.456789123 => dpos=5  dcount=14  val=123456789 x 10^-9
  //    123456789123    => dpos=12 dcount=12  val=123456789 x 10^0
  //    0.0123456789123 => dpos=1  dcount=13  val=123456789 x 10^-13
  //    .0123456789123  => dpos=0  dcount=12  val=123456789 x 10^-13
...
//sawritef("readflt: after calling unrdch*n"); checkpos(ch)
  ...
  exponent := exponent - dcount + ecount + dpos
...
AND writeflt(x, n, m) BE
{ // Write a floating point number x in a field of width n with
  // m digits after the decimal point.
  // m will be forced to be >=1
  // n will be forced to be >= m+3 to allow for a possible sign 
  //   and one digit before the decimal point.
  LET val = sys(Sys_flt, fl_unmk, x) // Upto 18 digits
  ...
  // otherwise val is an intger containing upto 18 (or 9) decimal digits
  // Its most and least significant digits are both non zero.
  LET digv = VEC 18 // will hold upto 19 extracted decimal digits.
  LET p = ?  // digv!0,.., digv!p will hold the extracted decimal digits.
  ...
  LET r = ?  // The position of the m^th digit after the decimal point,
             // ie the last digit to be output. It value will be q+m.
...
//writef("*nval=%n e=%n neg=%n*n", val, e, neg)
  // Extract the decimal digits into digv!0,..., digv!p
  p := -1

  { p := p+1
    digv!p := val MOD 10  // Extract the next digit
    val    := val  /  10
  } REPEATWHILE val

//newline()
//FOR i = 0 TO p DO writef(" %i2:%n", i, digv!i)
//writef(" e=%n*n", e)
  // 0 is the position of the least significant digit.
  // p is the position of the most  significant digit.
  // p is >= 0

  IF m<1 DO m := 1  // Ensure at least one fractional digit

  q := -e  // The position of the digit to the left of
           // the decimal point. If e=2 q will be -2
  // Position 0 corresponds to value digv!0 x 10^e

  r :=  q - m  // Position r holds the m^th digit after the
               // decimal point.

  // Round up the digits by adding 5 to the digit at position r-1,
  // if necessary.

  IF 1 <= r <= p+1 & digv!(r-1)>=5 DO
  { LET carry = 1
    LET i = r
    WHILE carry & i<=p DO
    { LET d = digv!i + carry
    ...
    IF carry DO     // Complete the conversion of
    { p := p+1      // eg  99999 to
      digv!p := 1   //    100000 
    }
  }
//writef("After rounding*n")
//FOR i = 0 TO p DO writef(" %i2:%n", i, digv!i)
//newline()
 
  s := q
  IF s < p DO s := p
  s := s+1 // Add one for the position of the sign character.
  // s is the position of the first character of the number.


  // Set t to the position of the first character to output
  // including padding. Remember the decimal point occurs between
  // positions q and q-1, so t-r+1 must be n-1.
  // Remember each position between t and r corresponds to a
  // character, but, in addition, the sign is output
  t := r + n - 2

  // If n was too small to include the sign, one digit before
  // the decimal point, the decimal point and m digits after the
  // decimal point, set t to the position of the sign character.
  IF t < p+1 DO t := p+1


  //writef("n=%n p=%n q=%n [", n, p, q)
  //FOR j = 0 TO 8 DO writef(" %n", digv!j)
  //writef(" ]*n")

//newline()
//writef("n=%n m=%n p=%n r=%n q=%n s=%n t=%n*n", n, m, p, r, q, s, t) 
...
AND writee(x, w, p) BE
{ // This functions needs modification to allow it to work with
  // 64-bit floating point numbers.
  // Write a floating point number in exponential form.
  // namely: Sd.dddddesdd
  // where
  // S is a sign: space or -
  // s is a signL + or -
  // d is a digit
  // w is the minimum field width, padding the output on the
  // left with spaces, if necessary.
  // p is the precision which is the number of digits to appear
  // after the decimal point. p must be between 1 and 9.
  // eg writee(-123.4567899, 13, 4) will output as -1.2346e+02
  // preceded by two spaces.
  // The number will occupy at least 3+p+4 charater positions.
  LET val = sys(Sys_flt, fl_unmk, x) // 10 digits plus exponent
  LET e = result2 + 8   // Exponent when val is treated as a scaled
                        // number with 8 digits after the decimal
                        // point. So val=012345678 and e=0
                        // represents 0.12345678
  LET digv = VEC 8
  LET neg = FALSE
  IF val<0 DO neg, val := TRUE, -val
  IF val=0 DO e := 0
  IF p<1   DO p := 1
  IF p>8   DO p := 8
//sawritef("writee: val=%n e=%n w=%n p=%n*n", val, e, w, p)

  // Output the padding spaces
  FOR i = p+8 TO w DO wrch(' ')

  // Zero is a special case -- This is not necessary
  //IF x#=0.0 DO
  //IF val=0 DO  // Corrected 18/08/2018
  //{ writes(" 0.")
  //  FOR i = 1 TO p DO wrch('0')
  //  writes("e+00")
  //  RETURN
  //}

  // The number is non zero, so extract 9 digits placing them
  // in digv!0 to digv!8.
...
//writef("digv -> [")
//FOR i = 0 TO 8 DO writef(" %n", digv!i)
//writes(" ]*n")

  { // Round the digits by adding 5 at at digit position p+1
    LET q = p+1
    LET carry = 0
    IF q<=8 DO carry := 5
    WHILE carry & q>=0 DO
    { LET sum = digv!q + carry
      digv!q := sum MOD 10
      carry  := sum  /  10
      q := q-1
    }
    ...
    { // Shift the digits to the right by one position
      // and place the carry in digv!0
      FOR i = 8 TO 1 DO digv!i := digv!(i-1)
      digv!0 := carry
    }
  }
  // The digits to output are at positions 0 to p.
  wrch(neg -> '-', ' ')  // Output the sign
  FOR i = 0 TO p DO      // Output the digits
  { wrch(digv!i + '0')
    IF i=0 DO wrch('.')  // Decimal point after the first digit
  }
  wrch('e')              // Output the exponent
  wrch(e>=0 -> '+', '-')
  writez(ABS e, 2)
}
