...
Possible value for fno have the following manifest constants.

GL_Init           // initialise SDL with everything
GL_SetFltScale    // Specify the integer that represents floating 1.0
GL_Quit           // Shut down SDL
GL_GetError       // str -- fill str with BCPL string for the latest GL error
GL_MkScreen       // width height
GL_SwapBuffers
GL_MkProg
GL_CompileVshader
GL_CompileFshader
...
GL_pollevent    // pointer to [type, args, ... ] to hold details of the next event
...
BCPLWORD glfn(BCPLWORD *a, BCPLWORD *g, BCPLWORD *W) {
  ...
  case gl_Init:
    #ifdef SDLavail
    printf("SDLavail is set\n");
    #endif
    #ifdef EGLavail
    printf("EGLLavail is set\n");
    #endif
    #ifdef GLavail
    printf("GLavail is set\n");
    #endif
...
#ifdef SDLavail
   // This system is using SDL to call GL
    { int argc = 0;
      BCPLWORD res = (BCPLWORD) SDL_Init(SDL_INIT_EVERYTHING);
    ...
    { // datav<32 the a vertex object is being used and datav is an offset
      // The are n vertex items each containing stride floating point values
      GLint loc = (GLint)a[1];
      GLint n = (GLint)a[2];
    ...
    case gl_DrawElements: // (mode, count, offset)
      ...
      // count  is the number of index values.
      // offset is the position in the index vector of the first
      //        vertex to use.

    { BCPLWORD mode   = a[1];
      GLsizei  count  = (GLsizei)(a[2]); // Number of index values to use.
      GLint*   offset = (GLint*)(a[3]);  // Offset (cast as a ptr) of the
                                         // first index element to use.
      GLenum   glmode = 0;

      //      printf("DrawElements: mode=%d count=%d offset=%d\n",
      //                      mode, a[2], a[3]);
      switch (mode) {
        default: return 0;

        case 1: glmode = GL_POINTS;         break;
        case 2: glmode = GL_LINES;          break;
        case 3: glmode = GL_LINE_STRIP;     break;
        case 4: glmode = GL_LINE_LOOP;      break;
        case 5: glmode = GL_TRIANGLES;      break;
        case 6: glmode = GL_TRIANGLE_STRIP; break;
        case 7: glmode = GL_TRIANGLE_FAN;   break;
      }

      glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 2);  // 2 is the index buffer object
      glDrawElements(glmode,
                     count,             // Number of vertices
                     GL_UNSIGNED_SHORT, // Type of index elements
                     offset);           // The position in bytes
    ...
    { GLushort *data = (GLushort *)&W[a[1]];
      ...
      //glEnable(GL_PRIMITIVE_RESTART);   // Pritive restart seems not to work.
