...
**
** (c) Copyright:  Martin Richards  30 Aug 2018
**
*/
...
{ int rc;
  ...
  for (i=1; i<argc; i++) {
    ...
    PRINTF("-v -v         As -v, but also include some Cincode level tracing\n");
  ...
  { int membytes = (memupb+tallyupb+vecstatsvupb+7)<<B2Wsh;
    ...
    if (boottrace>0)
      printf("Cintcode memory allocated by mmap\n");
#else
    W = PT malloc(membytes);
    if (boottrace>0)
      printf("Cintcode memory allocated by malloc\n");
#endif
  }
  ...
  W[rootnode+Rtn_system]       = 1;     // This is Cintsys
  ...
  W[rootnode+Rtn_icount]       = 100000000; // Estimated instructions per second.

  if (boottrace>0) PRINTF("Rootnode allocated at %d\n", rootnode);

  if (boottrace>0) PRINTF("Loading all resident programs and libraries\n");
  ...
  { BCPLWORD seg = loadseg("syscin/boot");
    if(seg==0) {
      ...
      PRINTF("environment variables such as BCPLROOT or BCPL64ROOT\n");
  ...
  */
  ...
  /* A debbugging aid!! To trace Cintcode execution */
  /* tracing = 1; */
...
{ // This version produces the same results as the other version
  ...
  // It is was used by the MDIV instruction (and the syslib muldiv function).
...
BCPLWORD doflt(BCPLWORD op, BCPLWORD a, BCPLWORD b, BCPLWORD c) {
  ...
  switch (op) {
    ...
    { // a=mantissa, b=exponent
      BCPLFLOAT da = (BCPLFLOAT) a;  // Why not use BCPLFLOAT???
      BCPLFLOAT res;
    ...
    case fl_unmk: // eg sys(Sys_flt, fl_unmk, 1.234)
                  ...
                  // Otherwise the result has up to 18 digits with an
                  // appropriate exponent. Neither the most and least
                  // significant digit os the matissa are zero.
                  // If TARGET64 is not defined, the manitissa will
                  // have no more than 9 digits.
    ...
    { BCPLINT64 mantissa;
      ...
      //printf("unmk2: d = %15.9e %d\n", d, exponent);

      // d > 10 divide by a power of 10 and adjust the exponent.
      ...
      mantissa = floor(d * 1e17); // Mantissa has 18 digits
      exponent -= 17;
    ...
    { 
#ifdef TARGET64
      BCPLFLOAT res = fmod(N2F a, N2F b);
#else
      BCPLFLOAT res = fmodf(N2F a, N2F b);
#endif
    ...
    case fl_acos:
    { BCPLFLOAT res = acos(N2F a);
      return F2N res;
    }

    case fl_asin:
    { BCPLFLOAT res = asin(N2F a);
      return F2N res;
    }

    case fl_atan:
    { BCPLFLOAT res = atan(N2F a);
      return F2N res;
    }

    case fl_atan2:
    { BCPLFLOAT res = atan2(N2F a, N2F b);
      return F2N res;
    }

    case fl_cos:
    { BCPLFLOAT res = cos(N2F a);
      return F2N res;
    }

    case fl_sin:
    { BCPLFLOAT res = sin(N2F a);
      return F2N res;
    }

    case fl_tan:
    { BCPLFLOAT res = tan(N2F a);
      return F2N res;
    }

    case fl_cosh:
    { BCPLFLOAT res = cosh(N2F a);
      return F2N res;
    }

    case fl_sinh:
    { BCPLFLOAT res = sinh(N2F a);
      return F2N res;
    }

    case fl_tanh:
    { BCPLFLOAT res = tanh(N2F a);
      return F2N res;
    }

    case fl_exp:
    { BCPLFLOAT res = exp(N2F a);
      return F2N res;
    }
