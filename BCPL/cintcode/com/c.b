...
13/10/2025
Scripts are looked up in directory $BCPLROOT/s if not
found in the current directory or the directories specified by
$BCPLSCRIPTS. So provided the scripts are in the standard place it is
not necessary to define the environment variable BCPLSCRIPTS.
...
3) The dirctory s/ in the BCPL root directory specified by the
   environment variable in the rtn_rootvar field of the rootnode.
4) The root directory specified by the environment variable in
   the rtn_rootvar field of the rootnode.

You can see what the rootnode environment variable names are set
to using the CLI command: setroot.
...
LET appendstring(s1, s2) = VALOF
{ // Append string s1 onto the end of s2 and return the
  // concatenated result.
  LET p = s2%0
  s2%0 := s1%0 + p
  FOR i = 1 TO s1%0 DO s2%(p+i) := s1%i
  RESULTIS s2
}
...
//sawritef("c: command-command filename=%s*n", cfile)
  ...
  UNLESS instream DO // As a last resort look in $BCPLROOT/s
  { LET fstr = VEC 256/bytesperword
    copystring("s/", fstr)
    appendstring(cfile, fstr)
    //writef("Looking for %s in the BCPL root directory*n", fstr)
    //instream := pathfindinput(fstr, rootnode!rtn_rootvar)
  }
