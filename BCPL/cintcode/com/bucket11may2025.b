...
MANIFEST {
  One  =    1_00000 // The constant 1.0 scaled with 5 decimal
...
{ writef("%9.3d currballno=%n*n", displaytime, currballno)
  ...
  writef("ax1=%9.5d ax2=%9.5d ax3=%9.5d*n", ax1, ax2, ax3)
...
//FOR i = 1 TO 10000000 DO i:=i+1
  ...
  writef("batx=%8.5d batxdot=%8.5d*n", batx, batxdot)
  writef("batxdotdot=%8.5d timediff=%8.3d*n",
          batxdotdot, displaytime-batpushtime)
//abort(5534)

  UNLESS activebat DO
    batxdotdot := batx < midx -> 20_00000, -20_00000

  IF activebat DO
  { // The bat is controlled by the computer.

    // If currballno=0 it attempts to selects a ball to hit. If
    // no ball is selected it chooses an acceleration towards
    // midx.

    // If there is a selected ball its parameters are placed
    // in selx, sely, selxdot and selectydot

    // If there is a selected ball and displaytime is less than
    // pushtimelimit, the bat continues accelerating with the
    // value in pushaccel.
    
    // If there is a selected ball and displaytime is not less
    // than pushtimelimit, a bat acceleration must be chosen.
    // If sely > pushlevel or ABS(selx-batx)<ballradius*4, it
    // sets a bat acceleration to cause the bat to move to
    // an x position ballradius*5 away from selx. This position
    // is on the midx side of the ball. The chosen acceleration
    // is placed in pushaccel and pushtimelimit is set to
    // displaytime+0_100, causing the acceleration to last for
    // 1/10 second.

    // The value of pushlevel is carefully chosen to ensure that
    // if a bat acceleration of 100.0 towards the ball set at the
    // time the ball falls below this level will cause the bat
    // to hit the ball as it touches the ground with suitable
    // force.

    // If sely<=pushylevel and displaytime is greater than
    // pushtimelimit, a bat acceleration of 100.0 towards the
    // ball is chosen and placed in pushaccel and pushtimelimit
    // set to the expected time the ball hits the ground.
    ...
    // If the cuurent ball is above the bucket base, it is
    // unselected.

    IF currballno=1 &
       cgy1>bucket_byc &
       bucket_lxc<cgx1<bucket_rxc DO currballno := 0
    IF currballno=2 &
       cgy2>bucket_byc &
       bucket_lxc<cgx2<bucket_rxc DO currballno := 0
    IF currballno=3 &
       cgy3>bucket_byc &
       bucket_lxc<cgx3<bucket_rxc DO currballno := 0

    
    // If currballno=0 try to select a ball that is not between
    // the bucket sides and above its base.
    //abort(6661)
    
    UNLESS currballno DO
    { UNLESS cgy1>bucket_byc &
             bucket_lxc<cgx1<bucket_rxc DO currballno := 1
      UNLESS cgy2>bucket_byc &
             bucket_lxc<cgx2<bucket_rxc DO currballno := 2
      UNLESS cgy3>bucket_byc &
    ...
    THEN { // There is no current selected ball, if its acceleration
	   ...
	   batxdotdot  := batx > midx -> -10_00000, 10_00000
	   batpushtime := 0
         }
    ELSE { // There is a selected ball. So select its details.
           IF currballno=1 DO
	   ...
	   // of the selected ball.
           ...
           { // Test the height of the selected ball
		    ...
		    { // Setup another push
		      batxdotdot :=  batx < selx -> +500_00000,
		                                    -500_00000
             ...
             ELSE { LET tx = midx       // To hold the target x position
	            ...
	            ELSE tx := selx + 8 * ballradius
                    ...
                    THEN batxdotdot :=
		          chooseaccel( tx, sely, selxdot,
                                       batx, batxdot,
                                       0, // Desired speed when
		                          // reaching target
		                       100_000 // Time to reach the target
				     )
  ...
  IF batx > wall_rx-batradius DO
    batx, batxdot, batxdotdot := wall_rx - batradius, -batxdot, -batxdotdot
  IF batx < batradius DO
    batx, batxdot, batxdotdot := batradius, -batxdot, -batxdotdot
...
{ // Allocate a surface for the bucket walls
  ...
  //writef("Calling mksurface width=%n height=%n*n", width, height)
...
//  moveto(cgx1, cgy1); drawto(300, 200)
...
{ LET stepmsecs = ?
  ...
  batx     := screen_xc              // x Position of bat
  baty     := floor_yt + batradius   // y Position of bat
