...
s_slct=26; s_of=27                   // Inserted 14/8/01
   ...
   { SWITCHON ch INTO
      ...
      { CASE '*p':
         ...
         CASE ':': rch()
              ...
              IF ch=':' DO { symb := s_of;  BREAK  } // Inserted 14/8/01
              symb := s_colon
              RETURN
...
{ dsw("AND", s_and)
   ...
   dsw("SLCT", s_slct)        // Inserted 14/8/01
...
{ LET stream = ?
   ...
   stream := pathfindinput(charv, "BCPLHDRS")
   TEST stream=0
...
{ LET a, op = 0, symb
   ...
   { DEFAULT: synerr("Error in expression")
      ...
      CASE s_slct: { LET len, sh, offset = 0, 0, 0  // Inserted 14/8/01
...
{ LET a, b, p = rbexp(), 0, 0
   ...
   { LET op = symb
      ...
      { DEFAULT:       RESULTIS a
         ...
         CASE s_of:     p := 8; ENDCASE // Inserted 14/8/01
...
//   writef("Undeclare %s*n", name+2)
   ...
   WHILE gdeflist DO { out2(h2!gdeflist, h3!gdeflist)
...
{ IF p>q DO { genr(f_j, dlab); RETURN }
...
{ LET offset = casek!r
  UNLESS offset RESULTIS r  // Nothing to do.
