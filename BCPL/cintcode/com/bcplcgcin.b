...
to allow faster assembly language interpreter.
...
LET cgdebughelp() BE
{writef("Values of debug for the BCPL to Cintcode codegenerator*n")
  writef("11 Output the compiled code (Cintcode or machine code)*n")
...
AND scan() BE
{ IF debug>=12 DO { writef("op=%t5 pdnop=%t5 ", opname(op), opname(pendingop))
                   dboutput()
                 }
  SWITCHON op INTO
  ...
  { DEFAULT:     cgerror("Bad OCODE op %n %s", op, opname(op))
    ...
    CASE s_lab:{ LET lab = rdl()
                 //IF debug>=11 & lab=102 DO abort(65432)
                 cgpendingop()
...
//  sawritef("Jump: After skipping STACKs op=%s*n", opname(op))
		 ...
		 // ie sometimes do not compile the jump
                 TEST op=s_lab
                 THEN { LET m = rdl()
		        //IF debug>=11 DO sawritef("*nJump L%n Lab L%n*n", l, m)
		        //IF debug>=11 & m=102 DO abort(87654)
                        UNLESS l=m DO
...
{ MANIFEST { upb=11 } // Max length of entry name
  ...
  IF debug>=11 DO writef("// Entry to:   %s*n", v)
  setlab(l)
  incode := TRUE
  forgetall()
}
...
{ // rlist is 0 or point to [link,addr,lab,?] where addr is the
  ...
  IF debug>=11 DO writef("%i4: L%n:*n", stvp, ln)
...
{ chkrefs(1)
  IF debug>=11 DO wrcode(f, "")
  codeb(f)
}

LET genb(f, a) BE IF incode DO
{ chkrefs(2)
  IF debug>=11 DO wrcode(f, "%i3", a)
  codeb(f)
  codeb(a)
}
...
{ chkrefs(3)
  IF debug>=11 DO wrcode(f, "%i3 %i3", a, b)
  codeb(f)
  codeb(a)
  codeb(b)
}
...
{ // flop is an operator whic can be the argument of FLTOP
  ...
  chkrefs(2)
  IF debug>=11 DO wrcode(f_fltop, "%s", flopname(flop))
  codeb(f_fltop)
  codeb(flop)
}
...
{ // This is only used to load a 64 bit floating point number
  ...
  { chkrefs(5)
    IF debug>=11 DO wrcode(f_mw, "#x%x8", mw)
    codeb(f_mw)
    code4b(mw)
  }

  chkrefs(5)
  IF debug>=11 DO wrcode(f_lw, "#x%x8", lw)
  codeb(f_lw)
  code4b(lw)
}
...
{ IF T64 & ON64 DO
  ...
  chkrefs(5)
  IF debug>=11 DO wrcode(f, "#x%x8", w)
  codeb(f)
...
{ // Only called by: genfb(f_fltop, fl_mk, exponent)
  chkrefs(3)
  IF debug>=11 DO wrcode(f, "%s %n", flopname(flop), a)
  codeb(f)
  codeb(flop)
  codeb(a)
}
...
{ chkrefs(4)
  IF debug>=11 DO wrcode(f, "%s %n %n", sfname(sfop), a, b)
  codeb(f)
  codeb(sfop)
  codeb(a)
  codeb(b)
...
AND codeh(h) BE
{ IF debug>=11 DO writef("%i4:  DATAH %n*n", stvp, h)
  code2b(h)
}

AND codew(wh, wl) BE TEST T64
THEN { // 64 bt target
       IF debug>=11 DO writef("%i4:  DATAW #x%x8%x8*n", stvp, wh, wl)
       TEST bigender
       THEN { codeb(wh>>24)
              codeb(wh>>16)
              codeb(wh>>8)
...
ELSE { // 32 bit target
       IF debug>=11 DO writef("%i4:  DATAW #x%x8*n", stvp, wl)
       TEST bigender
       THEN { codeb(wl>>24)
              codeb(wl>>16)
              codeb(wl>>8)
...
AND coder(ln) BE
{ LET labval = labv!ln
  IF debug>=11 DO writef("%i4:  DATAH L%n-$*n", stvp, ln)
  code2b(0)
...
{ LET r = rlist       // Assume RLIST ~= 0
  ...
  { skiplab := newlab()
    ...
    IF debug>=11 DO wrcode(f_j, "L%n", skiplab)
    codeb(f_j)
    codeb(0)
...
{ LET offset = (p-a)/2
  ...
  stv%(a+1) := offset
  UNLESS 0<=a<=64000 & 0<=offset<=255 | debug>=11 DO
...
{ // This is used as a debugging aid in scan to trace the
  ...
  IF debug=13 DO // Output the simulated stack
  { writef(" ssp=%n ", ssp)
