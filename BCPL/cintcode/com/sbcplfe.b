...
Cured bug concerning the closing of tostream when equal to stdout.
*/
...
{ LET treesize = 0
  ...
  targetbytelen := T64 -> 8, 4            // Set the target word length in bytes
...
//writef("BCPL hdrs = %s*n", hdrs)
  ...
  fromstream := findinput(argv!0)       // FROM/A
  sourcenamev!0 := argv!0    // Fileno zero is the FROM file
  sourcefileno  := 0

  IF fromstream=0 DO { writef("Trouble with file %s*n", argv!0)
  ...
  TEST argv!1                             // TO/K
  THEN { tostream := findoutput(argv!1)
         IF tostream=0 DO
  ...
  { LET b = VEC 64/bytesperword
    ...
    { // Start of loop to process each section
      ...
      IF debug=2 DO { writes("Parse Tree*n")
                      plist(tree, 0, 20)
                      newline()
                    }
