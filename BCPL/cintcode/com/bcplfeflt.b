...
Added the FLT tag to make floating point operations more convenient.

If FLT prefixes a name in a global, static, manifest, local or formal
parameter declaration, the name is given the FLT tag. If a name with
the FLT tag, a floating point constant or and expression with leading
operator #+, #-, #ABS, #*, #/, #MOD, #-> or FLOAT occurs as an operand
of +, -, ABS, *, /, MOD, =, ~=, <, <=, >, >= or -> it is replaced by
the floating point version.

An FLT context is any operand of FIX, #+, #-, #ABS, #*, #/, #MOD, #=,
#~=, #<, #<=, #>, #>=, #-> or an expression giving the value of a
static, manifest or local name having the FLT tag. If an expression
with leading operator +, -, ABS, *, /, MOD or -> occurs in an FLT
context, the operator is replaced by its floating point version. If an
integer constant occurs in an FLT context it is replaced by its
floating point equivalent.

These rules are applied repeatedly until there are no further changes
Their sole effect is to convert some integer operators to floating
point versions and some integer constants to floating point ones.
...
// Globals used in LEX
chbuf:feg
decval; FLT fltval; exponent; getstreams; charv
hdrs  // MR 10/7/04
...
{ LET treesize = 0
  ...
  writef("*nBCPL (10 Feb 2014) with FLT tag*n")
...
{ // Read an integer or floating point constant
  ...
  // or s_fnumber with the floating point value in fltval.
  ...
  { //writef("ch=%c zcount=%n pos=%n token=%n decval=%i4 exponent=%n*n",
    ...
    { DEFAULT: BREAK
      ...
      CASE '5': CASE '6': CASE '7': CASE '8': CASE '9':
        IF token=s_fnumber DO pos := pos+1
...
digitzero:
        IF token=s_fnumber DO pos := pos+1
        zcount := zcount+1 // Count of zeroes after a non zero.
      CASE '_':            // Ignore underlines
        ENDCASE

      CASE '.': 
        IF token=s_fnumber DO synerr("Bad floating point constant")
        token, pos := s_fnumber, 0
        // pos is the count of digits after the decimal point.
        ENDCASE
    }
    rch()
  }
//sawritef("readdecimal: token=%n decval=%n exponent=%n zcount=%n, pos=%n*n",
//          token, decval, exponent, zcount, pos)
...
//sawritef("readdecimal: making fnumber decval=%n exponent=%n zcount=%n, pos=%n*n",
//          decval, exponent, zcount, pos)
  // Correct the exponent
  exponent := exponent + zcount - pos
  fltval := sys(Sys_flt, fl_mk, decval, exponent)
//sawritef("*nToken=%n decval=%i4E%n fltval=%13e fltval=%13.3f fltval=%32b*n",
//          token, decval, exponent, fltval, fltval, fltval)
}
...
//sawritef("formtree: token=%n cis=%n*n", token, cis)
  ...
  { LET ln, name = ?, ?
    ...
    { DEFAULT:
      ...
      CASE s_fnumber:
        writef("token =%i3 ln=%i5 %12t %15e *n", token, ln, name, fltval)
        ENDCASE  
...
{ LET a, op = 0, token
   ...
   { DEFAULT: synerr("Error in expression")
      ...
      CASE s_fnumber:UNLESS -128<=exponent<=127 DO
                       synerr("Exponent of floating point constant out of range")
                     floatingchk()
                     a := mk2(s_fnumber, sys(Sys_flt, fl_mk, decval, exponent))
                     lex()
                     RESULTIS a
...
{ LET a, b, op, ln = 0, 0, token, lineno
  ...
  { DEFAULT: RESULTIS 0
    ...
    CASE s_name:CASE s_number:CASE s_fnumber:
...
{ LET size, ln = 0, 0
  ...
  { CASE s_number:
    ...
    CASE s_fnumber:writef("FNUMBER: %13e", h2!x);     RETURN
    ...
    CASE s_string:
      ...
      CASE s_fmul:CASE s_fdiv:CASE s_fmod:CASE s_fadd:CASE s_fsub:
...
{ DEFAULT:            writef("*nUnknown opname = %n*n", op)
  ...
  CASE s_fnumber:     RESULTIS "FNUMBER"
...
isflt; isconst; evalconst; transname; xref
nextlab; labnumber
newblk
...
//sawritef("Setting n=%x8(%i3)*n", n, n)
        ...
        //       it is s_nonflt the previous value was an integer.
...
{ // Return TRUE if expression x has the FLT tag,
  // ie if x is and fnumber, a name declared with the FLT tag
  // or has a leading operator such as #+ or #- that returns
  // a floating point value. Remember the operators such as + and
  // - are converted to #+ and #- if they have FLT oparands.
//sawritef("isflt: entered*n")
  SWITCHON h1!x INTO
  { DEFAULT:  RESULTIS FALSE

    CASE s_name: { LET c = cellwithname(x)
                   IF (h2!c & s_fltbit)=0 RESULTIS FALSE
//sawritef("Name: %s had FLT tag*n", @h3!x)
                   RESULTIS TRUE
                 }
...
//sawritef("load: case s_div*n")
         // Asymetric dyadic integer operators
         IF ff | isflt(x) DO
         { // Convert to floating point operators.
           h1!x := op=s_div -> s_fdiv,
                   op=s_mod -> s_fmod,
                               s_fsub
           load(x, TRUE)
           RETURN
         }
         load(h2!x, FALSE); load(h3!x, FALSE); out1(op)
         ssp := ssp - 1
         RETURN
                      
    CASE s_fdiv: CASE s_fmod:CASE s_fsub:
         load(h2!x, TRUE); load(h3!x, TRUE); out1(op)
         ssp := ssp - 1
         RETURN


    CASE s_ls: CASE s_gr: CASE s_le: CASE s_ge:
         // Asymetric dyadic integer relational operators
         IF ff | isflt(x) DO
         { // Convert to floating point operators.
           h1!x := op=s_ls -> s_fls,
                   op=s_gr -> s_fgr,
    ...
    CASE s_logand: CASE s_logor: CASE s_eqv: CASE s_neqv:
       { LET a, b = h2!x, h3!x
         TEST h1!a=s_name |
              h1!a=s_number THEN { load(b, FALSE); load(a, FALSE) }
    ...
    // Must separate s_number and s_fnumber.
    // and if ff=TRUE covert s_number to s_fnumber
...
{ DEFAULT:       sawritef("Syserr in cv2flt op=%s not in switch*n",
  ...
  CASE s_number: RESULTIS s_fnumber
...
{ // If ff=TRUE the expression x is to be evaluated in
  ...
  { CASE s_name: { LET c = cellwithname(x)
    ...
    CASE s_fnumber:RESULTIS h2!x

    CASE s_number: IF ff DO
                   { h1!x := s_fnumber
                     h2!x := sys(Sys_flt, fl_mk, h2!x, 0)
                   }
                   RESULTIS h2!x
  ...
  { CASE s_neg:    RESULTIS  -  a
    ...
    CASE s_fmod:   RESULTIS sys(Sys_flt, fl_modf,a,  b)
...
{ IF x=0 | y=0 DO { trnerr("Compiler error in assign")
  ...
  { CASE s_comma:  assign(h2!x, h2!y)
    ...
    CASE s_name: { LET c = cellwithname(x)
                   LET ff = h2!c & s_fltbit
                   IF isflt(y) DO ff := TRUE
                   // Evaluate the RHS in FLT context if necessary.
                   load(y, ff -> TRUE, FALSE)
                   transname(x, s_sp, s_sg, s_sl, 0, 0)
                   ssp := ssp - 1
                   RETURN
                 }
 
    CASE s_byteap: load(y,    FALSE)
                   load(h2!x, FALSE)
                   load(h3!x, FALSE)
                   out1(s_putbyte)
                   ssp:=ssp-3
    ...
    CASE s_rv:
    CASE s_vecap:  load(y, FALSE)
                   loadlv(x)
                   out1(s_stind)
                   ssp := ssp - 2
                   RETURN
 
    CASE s_of:   { LET slct = evalconst(h2!x, FALSE) // Inserted 11/7/01
                   LET len = slct>>24
                   LET sh  = slct>>16 & 255
                   LET offset = slct & #xFFFF
                   LET mask = -1
