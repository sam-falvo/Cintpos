// This is the BCPL compiler front end modified to accept the
// dialect of BCPL used on the BBC Microcomputer. It uses the
// standard codegenerator of the modern BCPL Cintcode System
// and can generated used with several either 32 or 64 bit
// Cintcode.
...
AND declsyswords() BE
{ dsw("AND", s_and);                  dsw("and", s_and)
  dsw("ABS", s_abs);                  dsw("abs", s_abs)
  dsw("BE", s_be);                    dsw("be", s_be)
  dsw("BITSPERBCPLWORD", s_bitsperbcplword)
  dsw("BREAK", s_break);              dsw("break", s_break)
  dsw("BY", s_by);                    dsw("by", s_by)
  dsw("CASE", s_case);                dsw("case", s_case)
  dsw("DO", s_do);                    dsw("do", s_do)
  dsw("DEFAULT", s_default);          dsw("default", s_default)
  dsw("EQ", s_eq);                    dsw("eq", s_eq)
  dsw("EQV", s_eqv);                  dsw("eqv", s_eqv)
  dsw("ELSE", s_else);                dsw("else", s_else)
  dsw("ENDCASE", s_endcase);          dsw("endcase", s_endcase)
  dsw("FALSE", s_false);              dsw("false", s_false)
  dsw("FINISH", s_finish);            dsw("finish", s_finish)
  dsw("FIX", s_fix);                  dsw("fix", s_fix)
  dsw("FLOAT", s_float);              dsw("float", s_float)
  dsw("FLT", s_flt);                  dsw("flt", s_flt)
  dsw("FOR", s_for);                  dsw("for", s_for)
  dsw("GOTO", s_goto);                dsw("goto", s_goto)
  dsw("GE", s_ge);                    dsw("ge", s_ge)
  dsw("GR", s_gr);                    dsw("gr", s_gr)
  dsw("GT", s_gr);                    dsw("gt", s_gr)
  dsw("GLOBAL", s_global);            dsw("global", s_global)
  dsw("GET", s_get);                  dsw("get", s_get)
  dsw("IF", s_if);                    dsw("if", s_if)
  dsw("INTO", s_into);                dsw("into", s_into)
  dsw("LET", s_let);                  dsw("let", s_let)
  dsw("LV", s_lv);                    dsw("lv", s_lv)
  dsw("LE", s_le);                    dsw("le", s_le)
  dsw("LS", s_ls);                    dsw("ls", s_ls)
  dsw("LT", s_ls);                    dsw("lt", s_ls)
  dsw("LOGOR", s_logor);              dsw("logor", s_logor)
  dsw("LOGAND", s_logand);            dsw("logand", s_logand)
  dsw("LOOP", s_loop);                dsw("loop", s_loop)
  dsw("LSHIFT", s_lshift);            dsw("lshift", s_lshift)
  dsw("MANIFEST", s_manifest);        dsw("manifest", s_manifest)
  dsw("MOD", s_mod);                  dsw("mod", s_mod)
  dsw("NE", s_ne);                    dsw("ne", s_ne)
  dsw("NEEDS", s_needs);              dsw("needs", s_needs)
  dsw("NEQV", s_xor);                 dsw("neqv", s_xor)
  dsw("NOT", s_not);                  dsw("not", s_not)
  dsw("OF", s_of);                    dsw("of", s_of)
  dsw("OR", s_else);                  dsw("or", s_else)
  dsw("QUERY", s_query);              dsw("query", s_query)
  dsw("RESULTIS", s_resultis);        dsw("resultis", s_resultis)
  dsw("RETURN", s_return);            dsw("return", s_return)
  dsw("REM", s_mod);                  dsw("rem", s_mod)
  dsw("RSHIFT", s_rshift);            dsw("rshift", s_rshift)
  dsw("RV", s_rv);                    dsw("rv", s_rv)
  dsw("REPEAT", s_repeat);            dsw("repeat", s_repeat)
  dsw("REPEATWHILE", s_repeatwhile);  dsw("repeatwhile", s_repeatwhile)
  dsw("REPEATUNTIL", s_repeatuntil);  dsw("repeatuntil", s_repeatuntil)
  dsw("SECTION", s_section);          dsw("section", s_section)
  dsw("SLCT", s_slct);                dsw("slct", s_slct)
  dsw("STATIC", s_static);            dsw("static", s_static)
  dsw("SWITCHON", s_switchon);        dsw("switchon", s_switchon)
  dsw("TO", s_to);                    dsw("to", s_to)
  dsw("TEST", s_test);                dsw("test", s_test)
  dsw("TRUE", s_true);                dsw("true", s_true)
  dsw("THEN", s_do);                  dsw("then", s_do)
  dsw("TABLE", s_table);              dsw("table", s_table)
  dsw("UNLESS", s_unless);            dsw("unless", s_unless)
  dsw("UNTIL", s_until);              dsw("until", s_until)
  dsw("VEC", s_vec);                  dsw("vec", s_vec)
  dsw("VALOF", s_valof);              dsw("valof", s_valof)
  dsw("WHILE", s_while);              dsw("while", s_while)
  dsw("XOR", s_xor);                  dsw("xor", s_xor)
  dsw("$", 0)
...
{ LET p = dvece + 3
  ...
  dvece := p
//  writef("addname: name cell at=%n %s k=%n a=%n dvect=%n*n",
//          dvece-3, @h3!name, k, a, dvect)
 //abort(1000)
}
...
{ // n is a name node not prefixed by FLT.
  ...
  { t := t - 3
    IF compstring(namestr, "m.he.savesize")=0 TEST h1!t
    THEN writef("Comparing m.he.savesize with %s*n", @h3!(h1!t))
    ELSE writef("m.he.savesize not found*n")
  } REPEATUNTIL h1!t=n | h1!t=0
