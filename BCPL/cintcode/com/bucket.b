...
28/05/2025
Made major change to use scaled arithmetic with 3 digits rater
...
MANIFEST {
  ...
  ag = -50_000     // Gravity acceleration
}
...
GLOBAL {
  ...
  starttime    // Set when starting becomes FALSE, initially zero
  reltime      // Time relative to starttime
  ...
  tscomploop
  tsabort
  ts               // Call a function without incrementing reltime
                   // done by modifying starttime.
  comploop         // Execute a busy loop
  tsabort
...
//writef("rx1dot=%10.3d ry1dot=%10.3*n", rx1dot, ry1dot)
//writef("t1dot =%10.3d w1dot =%10.3*n", t1dot,  w1dot)
    IF t1dot<=0 RETURN

    // Reverse t1dot with some loss of energy
    t1dot := rebound(t1dot)
//writef("t1dot =%10.3d w1dot =%10.3 after bounce*n", t1dot,  w1dot)

    // Transform back to (x,y) coordinates relative to cg
    rx1dot := inprod(t1dot,w1dot,  c,-s)
    ry1dot := inprod(t1dot,w1dot,  s, c)
//writef("rx1dot=%10.3d ry1dot=%10.3 after bounce*n", rx1dot, ry1dot)
    ...
    p2!2 := -rx1dot + cgxdot
    p2!3 := -ry1dot + cgydot
//writef("p1!2=%10.3d p1!3=%10.3d p2!2=%10.3d p2!3=%10.3d*n",
//        p1!2,       p1!3,       p2!2,       p2!3)
  ...
  { // Object 1 is the bat and object 2 is a ball
    ...
    p1!0 := p1!0 - muldiv(0_050, c, One)
    p1!1 := p1!1 - muldiv(0_050, s, One)
    p2!0 := p2!0 + muldiv(0_050, c, One)
    p2!1 := p2!1 + muldiv(0_050, s, One)
...
//writef("a=%8.3d  d=%8.3d  d^2=%8.3d*n", a, d, muldiv(d,d,One))
  ...
  c := muldiv(c, One, d)
//writef("dx=%10.3d  dy=%10.3d => cos=%8.3d sin=%8.3d*n", dx, dy, c, s)
...
{ newline()
  ...
  writef("cgx1=%9.3d cgy1=%9.3d cgx1dot=%10.3d cgy1dot=%10.3d*n",
          cgx1,      cgy1,      cgx1dot,      cgy1dot)
  writef("cgx2=%9.3d cgy2=%9.3d cgx2dot=%10.3d cgy2dot=%10.3d*n",
          cgx2,      cgy2,      cgx2dot,      cgy2dot)
  writef("cgx3=%9.3d cgy3=%9.3d cgx3dot=%10.3d cgy3dot=%10.3d*n",
          cgx3,      cgy3,      cgx3dot,      cgy3dot)

  IF currballno DO
  writef("selx=%9.3d sely=%9.3d selxdot=%10.3d selydot=%10.3d*n",
          selx,      sely,      selxdot,       selydot)
  //writef("ax1= %9.3d ax2= %9.3d ax3=    %10.3d*n", ax1, ax2, ax3)
  //writef("ay1= %9.3d ay2= %9.3d ay3=    %10.3d*n", ay1, ay2, ay3)
  writef("batx=%9.3d baty=%9.3d batxdot=%10.3d batxdotdot=%10.3d*n",
          batx, baty, batxdot, batxdotdot)
  writef("reltime=%8.3d pushtimelimit=% %8.3d*n",
          reltime,      pushtimelimit)
  IF currballno DO
  writef("bouncetime= %8.3d bouncex=%9.3d*n",
          bouncetime,     bouncex)
  //delay(2_000)
  //abort(3996)
}
...
//IF stepcount=88 DO
  ...
  IF started UNLESS finished DO
    IF bucket_byt < cgy1 < bucket_tyb &
       ...
       ABS cgy1dot < 2_000 &
       ABS cgy2dot < 2_000 &
       ABS cgy3dot < 2_000 DO finished := TRUE
...
//writef("reltime=%9.3d pushtimelimit=%9.3d pushaccel=%9.3d  push for %i5 msecs*n",
//        reltime,      pushtimelimit,      pushaccel, pushtimelimit-reltime)
      GOTO pushbat
    }
    ...
    { //writef("Ball %n is currently selected.*n")
      ...
      //writef("bucket_byc=%9.3d bucket_tyb=%9.3d cgy1=%9.3d cgy2=%9.3d cgy3=%9.3d*n",
      //        bucket_byc,      bucket_tyb,      cgy1,      cgy2,      cgy3)
      //writef("bucket_lxc=%9.3d bucket_rxc=%9.3d cgx1=%9.3d cgx2=%9.3d cgx3=%9.3d*n",
    ...
    { // No ball can be selected, so choose a small bat acceleration
      ...
      pushaccel := batx>midx -> -30_000,
                                +30_000
      batxdotdot := pushaccel
      pushtimelimit := reltime + 0_800
      //writef("No ball selected so set pushaccel=%9.3d towards midx and pushtimelimit=%9.3d*n",
   ...
   // writef("Calculating the time to bounce*n")
    ...
    bouncetime := 2_000  // Default values
    bouncex    := midx
    ...
    { LET t = 0_200 * i + i*i*4
      ...
      //writef("t= %9.3d: height=%9.3d*n", t, h-floor_yt-ballradius)
...
//    writef("Approx time to bounce=%9.3d  approx bouncex=%9.3d*n", bouncetime, bouncex)
//abort(1000)
    TEST bouncetime > 0_800
    THEN { // Try to place the bat at distance wall_rx/4 from the selected ball
	   ...
	   { pushtimelimit := reltime + 0_100
	     //writef("Positioning bat pushaccel=%10.3d reltime=%10.3d for %n msecs*n",
	     //                        pushaccel,       reltime,       pushtimelimit-reltime)
	   }
         }
    ELSE { // Accelerate the bat towards the ball for one sec
           TEST batx > selx THEN pushaccel := -700_000
	                    ELSE pushaccel :=  700_000
	   batxdotdot := pushaccel
           IF pushtimelimit < reltime DO
	     pushtimelimit := reltime + 1_000
         }
...
pushbat:
  ...
  //sps := 10
  //writef("applying batxdotdot=%9.3d to bat*n", batxdotdot)
  batxdot := batxdot + batxdotdot/sps
  batxdot := batxdot - batxdot/sps  // damping
  
  //writef("batx=%9.3d  batxdot=%9.3d  batxdotdot=%9.3d*n",
  ...
  //writef("Returning from step() with batx=%9.3d baty=%9.3d*n", batx, baty)
  //abort(4494)
}
...
{ selectsurface(@screen, screenxsize, screenysize)
  ...
  { //sawritef("blitsurf of bucket base*n")
    ...
    //sawritef("bucket_lxc=%9.3d bucket_byt)=%9.3d screenysize=%n*n",
    //          bucket_lxc,      bucket_byt, screenysize)
    blitsurf(@bucketbasesurf, @screen, bucket_lxc/One, bucket_byt/One-1)
  }

  // The bat
  //writef("Bat: batx=%9.3d baty=%9.3d*n", batx, baty)
  IF batsurfok DO
  ...
  { target_col := currballno=1 -> ball1colour,
    ...
    IF FALSE DO
    { IF target_t>0 DO
      { drawf(20, 100, "target_t=%9.3d", target_t)
        drawf(20,  70, "target_x=%9.3d", target_x)
      }
  ...
  IF debugging DO
  { drawf(30, 220, "Ball1 x=%10.3d  y=%10.3d xdot=%10.3d  ydot=%10.3d",
          cgx1, cgy1, cgx1dot, cgy1dot)
    drawf(30, 205, "Ball2 x=%10.3d  y=%10.3d xdot=%10.3d  ydot=%10.3d",
          cgx2, cgy2, cgx2dot, cgy2dot)
    drawf(30, 190, "Ball3 x=%10.3d  y=%10.3d xdot=%10.3d  ydot=%10.3d",
          cgx3, cgy3, cgx3dot, cgy3dot)
    drawf(30, 175, "Bat   x=%10.3d  y=%10.3d xdot=%10.3d",
          batx, baty, batxdot)
  }
}
...
{ // x     Initial position, must be >=0
  ...
  LET B   =  v
  LET C   =  x
  LET B2  = muldiv(B, B, 1_000)
  LET AC4 = 4 * muldiv(A, C, One)
  LET t   = muldiv(-B - sqrt(B2 - AC4), One, 2*A)
  //IF FALSE DO
  IF B2 < AC4  DO
  { writef("timetozero: System error B2=%9.3d AC4=%9.3d*n", B2, AC4)
    ...
    writef("B            = %12.3d*n", B)
    writef("C            = %12.3d*n", C)
    writef("B2           = %12.3d*n", B2)
    writef("AC4          = %12.3d*n", AC4)
    writef("B2-AC4       = %12.3d*n", B2-AC4)
    writef("sqrt(B2-AC4) = %12.3d*n", sqrt(B2-AC4))
    writef("t            = %12.3d*n", t)
    //writef("muldiv(-25_000, 328_968, 1_000)**4 = %12.3d*n",
    //        muldiv(-25_000, 328_968, 1_000)*4)
    abort(1000) 
...
{ writef("root 2 = %12.3d*n", sqrt(2_000))
  ...
  { // Code to test the cosines function
    ...
    { LET c, s, rsq = ?, ?, ?
      ...
      writef("dx=%9.3d  dy=%9.3d cos=%9.3d sin=%9.3d rsq=%9.3d*n",
              One, dy, c, s, rsq)
      IF e1 < rsq DO e1 := rsq
      IF e2 > rsq DO e2 := rsq
    }
  ...
  starting  := FALSE | TRUE  // Trap door open if TRUE
  started   := FALSE
  finished  := FALSE
  starttime := -1
