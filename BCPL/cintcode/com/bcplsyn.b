...
18/08/2026
Changed the values of debug so that 1-9 are aids for the frontend and
10-20 are for codegenerator debugging aids. -d 99 will output the
debug values for the current compiler. It calls cgdebughelp to output
the values for the current codegenerator.
...
{ writef("*nSummary of the valid -d values:*n*n")
  ...
  TEST (cgdebughelp & #xFFFF0000) = globword
  THEN writef("cgdebughelp is not defined in this codegenerator*n")
  ELSE cgdebughelp()
...
//writef("stdout=%n stdin=%n*n", stdout, stdin)
  ...
  debug := 0  // debug holds the compiler debugging level.
              // Try bcpl com/hello.b -d 99 to see what is available
...
// *-h/S,-d/N"
  ...
  IF argv!11 DO
    defaultencoding := GB2312     // GB2312/S
  IF argv!12 DO
    defaultencoding := UTF8       // UTF8/S
  encoding := defaultencoding
  IF argv!13 DO
    savespacesize := !(argv!13)   // SAVESIZE/K/N
  ...
  noselst := argv!19              // NOSELST/S
				  ...
				  // It uses other Cintcode instructions.
  ...
  IF T16 DO
  { T16, T32, T64 :=  TRUE, FALSE, FALSE
    targetbytelen, targetbitlen :=  2, 16
    IF T32 | T64 DO
  ...
  IF T32 DO
  { T32, T64 := TRUE, FALSE
    targetbytelen, targetbitlen := 4, 32
    IF T64 DO
    { writef("Only one of T16, T32 or T64 is allowed*n")
      RESULTIS 0
    }
  ...
  IF T64 DO
  { T32, T64 :=  FALSE, TRUE
    targetbytelen, targetbitlen :=  8, 64
  }

  UNLESS T16 | T32 | T64 TEST ON64
  THEN { T32, T64 := FALSE, TRUE
         targetbytelen, targetbitlen := 8, 64
       }
  ELSE { T32, T64 := TRUE, FALSE
         targetbytelen, targetbitlen :=  4, 32
       }

  writef("*n%n bit BCPL (18 Apr 2026), %n bit target*n",
          bitsperword, targetbitlen)

help:
  IF helping DO
  { newline()
    writef("The compiler arguments TREE, TREE2, *
           *D1, D2 and -oc have*n")
    writef("have been removed and replaced by the *
           *-d option.*n")
    writef("Try bcpl com/hello.b to junk -d 99 to *
           *see what is now*n")
    writef("available. Try this command with 99 replaced by *
           *all values*n")
    writef("from, say, 1 to 20.*n")
    writef("The argument VER and OPT has been renamed *
           *ERR and DEFS.*n")
    writef("The debug values beteen 1 and 9 cause debug *
           *information*n")
    writef("to be generated by the frontend and values*n")
    writef("between 10 and 19 cause debug information *
	   *output from*n")
    writef("the current codegenerator*n*n")
    errcount := 0
    prvaliddebugvalues()
    GOTO fin
  }
...
IF noselst DO writef("NOSELST option was given*n")
  ...
  { LET b = VEC 64/bytesperword+1
	...
	selectoutput(stdout)
      ...
      IF debug=5 DO
      { newline() //writef("Write OCODE in readable form*n")
        procode()
      }
