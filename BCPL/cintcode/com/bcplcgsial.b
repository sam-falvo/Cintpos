...
AND scan() BE
{ IF debug>=12 DO { writef("OP=%i3 PND=%i3 ", op, pendingop)
                    dboutput()
                  }
...
{ IF debug>=11 DO wrcode(f, a)
  codef(f)
  codep(a)
}

LET genfkp(f, k, a) BE IF incode DO
{ IF debug>=11 DO wrcode(f, k, a)
  codef(f)
  codek(k)
  codep(a)
}
...
{  IF debug>=11 DO wrcode(f, n)
   codef(f)
   codeg(n)
}

LET genfkg(f, k, n) BE IF incode DO 
{  IF debug>=11 DO wrcode(f, k, n)
   codef(f)
   codek(k)
   codeg(n)
}
