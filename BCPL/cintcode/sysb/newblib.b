...
/*
11/02/16
Added BLIB function stackfree() to return approximately how much of the
run time stack is currently unused.
...
{ MANIFEST
  ...
  //LET keyword  = VEC 30
  LET chvec    = VEC 255/bytesperword
...
//sawritef("*nrdargs: First free position in argv is %n*n", w)

  { // Main loop
    LET argno = -1
    LET wupb = upb - (w - argv) // Number of words remaining in argv
    LET itemtype = rditem(w, wupb)
    //LET itemtype = rditem(chvec, 255) // MR 14/02/2016

    //clear_words(keyword, 31) // Clear keyword!0 to keyword!30 // MR 14/02/2016
...
//abort(4444)
                 ...
                 getkeylen(keys, len, i, chvec) // Get the keyword
                 writes(chvec)
...
//abort(8888)
                 ...
                 itemtype := rditem(chvec, 255) // MR 14/02/2016
//abort(9999)
...
//abort(7777)
                           ...
                           { // It is a switch parameter
                             argv!i := FALSE
                             IF compstring(chvec, "yes") = 0 |
                                compstring(chvec, "y") = 0 DO argv!i := TRUE
...
// Return approximately how many words of stack are currently unused.
AND stackfree(x) = VALOF
{ LET usedwords = @x - currco
  LET freewords = currco!co_size - usedwords
  result2 := usedwords
  RESULTIS freewords
}
