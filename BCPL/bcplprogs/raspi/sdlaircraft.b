...
{ // Rotate the aircraft clockwise by angle radians about axis t,
  // adjusting wdot and ldot appropriately.
  LET FLT cosa = sys(Sys_flt, fl_cos, angle)
  LET FLT sina = sys(Sys_flt, fl_sin, angle)

  // For the aircraft orientation. compute
  // ( 1  0  0 )   ( ctn ctw cth )
  // ( 0  c  s ) x ( cwn cww cwh )
  // ( 0 -s  c )   ( cln clw clh )
  
  LET FLT ww =  cosa*cww + sina*clw
  LET FLT lw = -sina*cww + cosa*clw

  LET FLT wh =  cosa*cwh + sina*clh 
  LET FLT lh = -sina*cwh + cosa*clh

  // For the velocity adjustment, compute
  // ( c -s ) x ( wdot )
  // ( s  c )   ( ldot )
  
  LET FLT newwdot = cosa*wdot - sina*ldot
  LET FLT newldot = sina*wdot + cosa*ldot
...
//writef("tdot=%9.5f wdot=%9.5f ldot=%9.5f)*n", tdot, wdot, ldot)
//writef("gives*n")
  cww, cwh :=  ww, wh
  clw, clh :=  lw, lh
...
{ // Rotate the aircraft clockwise by angle radians about axis w,
  // adjusting ldot and tdot appropriately.
  ...
  LET FLT tn = cosa*ctn - sina*cln
  LET FLT ln = sina*ctn + cosa*cln

  LET FLT th = cosa*cth - sina*clh 
  LET FLT lh = sina*cth + cosa*clh

  // For the velocity adjustment, compute
  // ( c -s ) x ( ldot )
  // ( s  c )   ( tdot )
  
  LET FLT newldot = ldot*cosa - tdot*sina
  LET FLT newtdot = ldot*sina + tdot*cosa
...
{ // Rotate the aircraft clockwise by angle radians about axis l,
  // adjusting tdot and wdot appropriately.
  ...
  LET FLT tn =  cosa*ctn + sina*cwn
  LET FLT wn = -sina*ctn + cosa*cwn

  LET FLT tw =  cosa*ctw + sina*cww 
  LET FLT ww = -sina*ctw + cosa*cww
  
  // For the velocity adjustment, compute
  // ( c -s ) x ( tdot )
  // ( s  c )   ( wdot )
  
  LET FLT newtdot = cosa*tdot - sina*wdot
  LET FLT newwdot = sina*tdot + cosa*wdot
...
{ // The forces on the aircraft are as follows.
  ...
  // ie about 2ft behind the origin. It is applied using applyforce.
  // It affect fl and rfw
...
//writef("Thrust force: %13.3f => ft =%13.3f*n", thrust, ft)
  ...
  rft := rft + 200.0 * aileron * tdot / steprate
...
//abort(1789)
  ...
  // Rotate the aircraft correcting tdot, wdot and ldot.
...
//writef("Trim rudder right %6.3f*n", rudder)
      ...
      CASE sdle_arrowup:
                c_trimelevator := c_trimelevator + 0.10
      ...
      CASE sdle_arrowright:
                c_trimaileron := c_trimaileron + 0.20
...
//rtdot := 1.0  // Debugging value
  ...
  { // Read joystick and keyboard events
    ...
    FOR i = 1 TO 3 IF stepping UNLESS crashed DO step()
