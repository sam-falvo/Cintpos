...
{ IF started UNLESS finished DO
  ...
  { LET t = sdlmsecs()
    ...
    { // Choose a new random target x position every half second
      LET xmax = screenxsize*One
      targetx := randno(xmax)

      IF randno(100)<90 DO
      { // About 90% of the time the target x position
        // depends on the x position of the lowest ball.
        LET dx = 100_00000
        LET minx, miny = cgx1, cgy1

        IF cgy2<miny DO minx, miny := cgx2, cgy2
        IF cgy3<miny DO minx, miny := cgx3, cgy3

        TEST batx < minx
        THEN targetx := minx + dx
        ELSE targetx := minx - dx

        IF targetx <= 0 DO targetx := 5_00000
        IF targetx >= screenxsize*100000 DO
             targetx := (screenxsize - 10) * 100000

        //writef("minx=%10.5d miny=%10.5d dx=%10.5d batx=%10.5d targetx=%10.5d*n",
        //        minx, miny, dx, batx, targetx) 
      }
      randbattime := t
    }

    // Accelerate towards targetx
    TEST batx > targetx THEN abatx := -1000_00000
                        ELSE abatx :=  1000_00000
  }

  // Apply forces to the bat
  batxdot := batxdot + abatx/sps
  IF batxdot> 600_00000 DO batxdot :=  300_00000
  IF batxdot<-600_00000 DO batxdot := -300_00000
...
{ selectsurface(screen, screenxsize, screenysize)
  ...
  IF randombat DO
  { setcolour(bucketcolour)
    drawfillcircle(targetx/One, baty/One, 5)
  }
  ...
  //plotf(targetx/100000, 80, "x") // Debugging aid
