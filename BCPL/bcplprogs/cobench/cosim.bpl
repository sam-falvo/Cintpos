...
GLOBAL $(
  ...
  priqupb:ug+1    // The upper bound
  priqn:ug+2         // Number of items in the priority queue
  wkqv:ug+3       // The vector of work queues
  count:ug+4      // count of messages processed
  nodes:ug+5      // The number of nodes
  ptmax:ug+6      // The maximum processing time
  stopco:ug+7     // The stop coroutine
  cov:ug+8        // Vector of message coroutines
  ranv:ug+9       // A vector used by the random number generator
  rani:ug+10; ranj:ug+11 // subscripts of ranv
  simtime:ug+12    // Simulated time
  stoptime:ug+13   // Time to stop the simulation
  tracing:ug+14
...
LET rnd(n) = VALOF
$( LET val = (ranv!rani + ranv!ranj) & #xFFFFFFF
//^1004310
  ranv!rani := val
  rani := (rani + 1) REM 55
  ranj := (ranj + 1) REM 55
...
$( LET a, b = #x2345678+seed, #x5362781
//^1
  ranv := getvec(54)
...
//  ^0
  ...
  nodes, stoptime, ptmax := 500, 1000000, 1000
//^1
  IF argv!0 & string.to.number(argv!0) DO nodes    := result2 // -k
//   ^1       ^0                          ^0
  IF argv!1 & string.to.number(argv!1) DO stoptime := result2 // -s
//   ^1       ^0                          ^0
  IF argv!2 & string.to.number(argv!2) DO ptmax    := result2 // -p
//   ^1       ^0                          ^0
  IF argv!3 & string.to.number(argv!3) DO seed     := result2 // -r
//   ^1       ^0                          ^0
  tracing := argv!4                                           // -t
//^1
...
// Total number of Cintcode instrctions executed: 435,363,350
