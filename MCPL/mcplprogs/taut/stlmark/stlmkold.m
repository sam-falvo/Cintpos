...
: x => writef "m0 called\n"
       ...
       newline()
       LET y = simp x
       writef("Simplified tree:\n")
       prtree (y, 0, 20)
       newline()
       RETURN TRUE

// The function simp removes Not from the expression, except possibly
// as the leading operator.  I don't think this is worth doing since
// Not operators are removed in the normal operation of the algorithm.
// However, I have left this unused function in.

FUN simp
: [Eqv, x, y] => simp[And, [Imp, x, y], [Imp, y, x]]

: [Imp, x, y] => MATCH (simp x, simp y)
                 : [Not,a],  [Not,b] => mk3(Imp, b, a)
                 : [Not,a],       b  => mk3(Or,  a, b)
                 : a,        [Not,b] => mk2(Not, mk3(And, a, b))
                 : a,             b  => mk3(Imp, a, b) 
                 .
: [Or,  x, y] => MATCH (simp x, simp y)
                 : [Not,a],  [Not,b] => mk2(Not, mk3(And, b, a)) 
                 : [Not,a],       b  => mk3(Imp, a, b)
                 : a,        [Not,b] => mk3(Imp, b, a)
                 : a,             b  => mk3(Or,  a, b) 
                 .
: [And, x, y] => MATCH (simp x, simp y)
                 : [Not,a],  [Not,b] => mk2(Not, mk3(Or, a, b))
                 : [Not,a],       b  => mk2(Not, mk3(Imp, b, a))
                 : a,        [Not,b] => mk2(Not, mk3(Imp, a, b))
                 : a,             b  => mk3(And, a, b) 
                 .
: [Not,    x] => MATCH (simp x)
                 : [Not, a]          => a
